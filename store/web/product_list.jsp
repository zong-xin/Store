		...
		<div style="width:380px;margin:0 auto;margin-top:50px;">
			...
			<c:if test="${not empty pageBean && not empty pageBean.list}">
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a href="${pageContext.request.contextPath}/product?methodName=searchPlistByCid&pageNumber=${pageBean.pageNumber-1}&cid=${param.cid}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
					</c:otherwise>
				</c:choose>
				<!-- 当前页 和其他页码 -->
				<c:forEach begin="1" end="${pageBean.totalPage}" var="page">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="${pageContext.request.contextPath}/product?methodName=searchPlistByCid&pageNumber=${page}&cid=${param.cid}">${page}</a></li>
						</c:otherwise>
						</c:choose>
				</c:forEach>
				<!-- 下一页 -->
				<c:choose>
					...
					<c:otherwise>
						<li><a href="${pageContext.request.contextPath}/product?methodName=searchPlistByCid&pageNumber=${pageBean.pageNumber+1}&cid=${param.cid}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
					</c:otherwise>
				</c:choose>
